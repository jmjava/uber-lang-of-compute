...
Runnable local lab for the full in-cluster stack: **CRDs**, **kbl-controller**, **kbl-tsdb**, **Volcano** batch scheduling, and sample finance workloads.

AWS production deployment is scaffolded separately under `infra/aws/cdk/` (see [ADR 0026](../docs/adr/0026-kind-lab-aws-cdk.md)). Volcano integration in the lab is documented in [ADR 0029](../docs/adr/0029-volcano-kind-lab.md).
...
Skip Volcano (controller + TSDB + workflow only):
...
```bash
KBL_VOLCANO_VERSION=v1.9.0 ./lab/scripts/up.sh
```
...
| VCJob `julia-finance-volcano` | `default` | Julia init-chain via Volcano scheduler |
...
The Volcano demo (`lab/manifests/volcano/`) runs the same Julia chain without a DominoChain CR — it exercises the blog's Provisioning-layer scheduler path.
...
lab/
  ...
  manifests/volcano/          # Queue + snapshot + Julia VCJob
